...
def get_hotel_url(hotel_name, checkin_date=None, checkout_date=None):
    ...
    lang_param = "&hl=id"  # Add this parameter
    
    return base_url + date_params + lang_param
        ...
        while len(reviews_data) < max_reviews and scroll_attempts < max_scroll_attempts:
            ...
            for review in reviews[last_review_count:]:
                ...
                try:
                    ...
                    # Get rating 
                    rating = review.find_element(By.CSS_SELECTOR, "div.GDWaad").text
...
def save_to_json(hotel_data, json_file="hotels_data.json"):
...
def extract_hotel_data(driver, hotel_name, hotel_lookup):
    ...
    try:
        ...
        # Extract reviews with more accurate count from explicit span (e.g., "4.888 ulasan")
        def parse_reviews_count(raw: str):
            text = raw.lower().replace("ulasan", "").strip()
            text = text.replace("(", "").replace(")", "")
            # Handle formats like "4.9 rb"
            if "rb" in text:
                ...
                try:
                    ...
                    pass
            # Handle formats like "4.888" (ID thousand separator)
        ...
        reviews_count = "N/A"
        reviews_elem_precise = soup.select_one("span.P2NYOe.GFm7je.sSHqwe")
        if reviews_elem_precise:
            parsed = parse_reviews_count(reviews_elem_precise.text)
        ...
        parent_categories = {
            ...
            'Konservasi air': [],
            'Pengurangan sampah': [],
            'Pengambilan sumber daya yang berkelanjutan': []
        }
...
def main():  
    ...
    options.add_argument('--accept-lang=id-ID,id;q=0.9')
    prefs = {
    "intl.accept_languages": "id,id-ID",
    ...
    if Path("hotels_data.csv").exists():  
        Path("hotels_data.csv").unlink()  
    ...
    try:
        ...
        first_write = True
        checkin = "2025-12-28"  # Format: YYYY-MM-DD
        checkout = "2025-12-29"  # Format: YYYY-MM-DD
        
        # Hapus file JSON sebelumnya jika ada
        if Path("hotels_data.json").exists():
            Path("hotels_data.json").unlink()
